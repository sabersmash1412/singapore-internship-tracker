...
def category(title):
    patterns = [
        ("Data & AI", r"\b(data|analytics|machine learning|ai|ml)\b"),
        ...
        ("IT & Infrastructure", r"\b(it|information technology|infrastructure|cloud|network|systems)\b"),
    ]
...
def eligible(job):
    return (
        ...
        and category(job["title"]) is not None
    )
...
def enrich(job):
    ...
    job["category"] = category(job["title"])
    ...
    match = PERIOD.search(job["title"])
    evidence = job["title"] if match else None
    if not match:
        for sentence in re.split(r"(?<=[.!?])\s+", description):
            if re.search(r"\b(internship|intern|availability|start date|intake)\b", sentence, re.I):
                match = PERIOD.search(sentence)
                if match:
                    evidence = sentence
                    break
    job["period"] = match.group(0) if match else None
    ...
    job["duration"] = duration.group(1) if duration else None
    job["duration_evidence"] = duration.group(0) if duration else None
